...
# Crear máscara para quitar Santa Rosa de Cabal en todas sus formas
...
# ¿Cuántos hombres y mujeres hay inscritos en total?
...
print(f"→ Mujeres admitidas: {pct_mujeres_adm:.2f}%")
print(f"→ Hombres admitidos: {pct_hombres_adm:.2f}%")

"""$visualizaciones$"""

# Agrupamos inscritos por programa y sexo
programa_counts = Base_inscritos.groupby(["PROGRAMA ACADÉMICO","SEXO"])["INSCRITOS"].sum().unstack(fill_value=0)

# Seleccionamos los 15 programas con más inscritos
...
programa_pct_reset = programa_pct.reset_index().melt(id_vars="PROGRAMA ACADÉMICO",
...
fig.update_layout(title="Comparativa Hombres vs Mujeres por Programa Académico (Top 10)",
...
# Agrupar inscritos por año y sexo
evolucion = Base_inscritos.groupby(["AÑO", "SEXO"])["INSCRITOS"].sum().reset_index()

# Calcular participación relativa (% mujeres y % hombres por año)
totales = evolucion.groupby("AÑO")["INSCRITOS"].transform("sum")
...
fig = px.line(evolucion,
              ...
              title="Tendencia de participación femenina vs masculina en el tiempo")

fig.update_traces(mode="lines+markers", line=dict(width=3))
fig.update_layout(yaxis_title="Participación (%)", xaxis_title="Año")
fig.show()
...
print("------ Antes de la eliminación de Santa Rosa -----")
...
print("\n------ Después de la eliminación de Santa Rosa -----")
print(Base_admitidos['MUNICIPIO'].value_counts())

#unificar los pereiras

print("\n------ Antes de la unificación de Pereira -----")
print(Base_admitidos['MUNICIPIO'].value_counts())
...
print("\n------ Después de la unificación de Pereira -----")
print(Base_admitidos['MUNICIPIO'].value_counts())
...
#Contar cuántos hombres y cuántas mujeres se matricularon.
...
# --- Gráfico de barras: Hombres vs Mujeres Admitidos ---
...
fig = px.bar(
    ...
    text='Porcentaje',
    color_discrete_map={'Hombre': 'steelblue', 'Mujer': 'orange'},
    title='Comparación Hombres vs Mujeres Admitidos'
)
...
# --- Gráfico de evolución: Hombres y Mujeres Admitidos por Año ---
Base_admitidos['AÑO'] = pd.to_numeric(Base_admitidos['AÑO'], errors='coerce')
evolucion_adm = Base_admitidos.groupby(["AÑO", "SEXO"])["ADMITIDOS"].sum().reset_index()
totales_anuales = evolucion_adm.groupby("AÑO")["ADMITIDOS"].transform("sum")
...
fig = px.line(
    ...
    markers=True,
    color_discrete_map={"Hombre": "steelblue", "Mujer": "orange"},
    title="Evolución de la participación de Hombres y Mujeres Admitidos por Año"
)

fig.update_traces(mode="lines+markers", line=dict(width=3))
fig.update_layout(
    yaxis_title="Porcentaje dentro del año (%)",
    xaxis_title="Año",
    legend_title="Sexo",
    hovermode="x unified"
)
...
# --- CARRERA CON MÁS ADMITIDOS POR GÉNERO (Donut Chart) ---

# Calculate top 10 programs for women and men based on admitted students
admitidos_genero = Base_admitidos.groupby(["PROGRAMA ACADÉMICO", "SEXO"])["ADMITIDOS"].sum().reset_index()
...
fig_mujer = px.pie(
    top_mujer,
    names='PROGRAMA ACADÉMICO',
    values='ADMITIDOS',
    hole=0.4,
    title='Top 15 Carreras con Más Mujeres Admitidas',
...
fig_hombre = px.pie(
    ...
    values='ADMITIDOS',
    hole=0.4,
    title='Top 15 Carreras con Más Hombres Admitidos',
    color_discrete_sequence=px.colors.sequential.Blues
)
fig_hombre.show()

# Agrupamos admitidos por programa y sexo
programa_counts_adm = Base_admitidos.groupby(["PROGRAMA ACADÉMICO","SEXO"])["ADMITIDOS"].sum().unstack(fill_value=0)

# Seleccionamos los 15 programas con más admitidos
...
# Convertimos a formato largo para Plotly
programa_pct_reset_adm = programa_pct_adm.reset_index().melt(id_vars="PROGRAMA ACADÉMICO",
                                                     value_vars=["Hombre","Mujer"],
                                                     var_name="Sexo", value_name="Porcentaje")
...
fig_adm.update_layout(title="Comparativa Hombres vs Mujeres por Programa Académico (Admitidos - Top 15)",
                  xaxis_title="Programa Académico",
                  yaxis_title="Porcentaje dentro del programa (%)",
                  xaxis_tickangle=-45,
                  legend_title="Sexo",
...
# --- Gráfico de evolución: Hombres y Mujeres Admitidos por Año ---
Base_Matriculados['AÑO'] = pd.to_numeric(Base_Matriculados['AÑO'], errors='coerce')
evolucion_matric = Base_Matriculados.groupby(["AÑO", "SEXO"])["MATRICULADOS"].sum().reset_index()
totales_anuales = evolucion_matric.groupby("AÑO")["MATRICULADOS"].transform("sum")
...
fig = px.line(
    ...
    markers=True,
    color_discrete_map={"Hombre": "steelblue", "Mujer": "orange"},
    title="Evolución de la participación de Hombres y Mujeres Matriculados por Año"
)
...
matriculados_genero = (
    Base_Matriculados
    .groupby(["PROGRAMA ACADÉMICO", "SEXO"])["MATRICULADOS"]
    .sum()
    .reset_index()
)
...
fig_mujer = px.pie(
    ...
    values='MATRICULADOS',
    hole=0.4,
    title=' Top 15 Carreras con Más Mujeres Matriculadas 👩',
    color_discrete_sequence=px.colors.sequential.Oranges
)
...
fig_hombre = px.pie(
    ...
    values='MATRICULADOS',
    hole=0.4,
    title='Top 15 Carreras con Más Hombres Matriculadas',
    color_discrete_sequence=px.colors.sequential.Blues
)
fig_hombre.show()

# Agrupamos matriculados por programa y sexo
programa_counts_matric = Base_Matriculados.groupby(["PROGRAMA ACADÉMICO","SEXO"])["MATRICULADOS"].sum().unstack(fill_value=0)

# Seleccionamos los 15 programas con más matriculados
...
# Convertimos a formato largo para Plotly
programa_pct_reset_matric = programa_pct_matric.reset_index().melt(id_vars="PROGRAMA ACADÉMICO",
                                                     value_vars=["Hombre","Mujer"],
                                                     var_name="Sexo", value_name="Porcentaje")
...
fig_matric.update_layout(title="Comparativa Hombres vs Mujeres por Programa Académico (Matriculados - Top 15)",
                  xaxis_title="Programa Académico",
                  yaxis_title="Porcentaje dentro del programa (%)",
                  xaxis_tickangle=-45,
                  legend_title="Sexo",
...
# Datos dinámicos desde tus variables
...
fig.update_layout(
    title_text='Comparación porcentual por género y etapa',
...
# Crear figura única con ambos géneros
fig = go.Figure()
...
fig.update_layout(
    title='Comparación porcentual por género y etapa',
    polar=dict(
        radialaxis=dict(visible=True, range=[40, 55])
    ),
...
Este código sirve para predecir y visualizar tendencias de equilibrio de género a partir de los datos históricos de inscripción, apoyando el análisis y la toma de decisiones en temas de igualdad y participación femenina.
"""

# 1) Crear una sola base de datos
df_all = Base_inscritos[['AÑO', 'PROGRAMA ACADÉMICO', 'SEXO', 'INSCRITOS']]

# 2) Calcular total de mujeres por año y programa
df_mujeres = (
    df_all[df_all['SEXO'] == 'Mujer']
    .groupby(['AÑO', 'PROGRAMA ACADÉMICO'])['INSCRITOS']
    .sum()
    .reset_index(name='Mujeres')
)

# 3) Calcular total general por año y programa
df_total = (
    df_all.groupby(['AÑO', 'PROGRAMA ACADÉMICO'])['INSCRITOS']
    .sum()
    .reset_index(name='Total')
)

# 4) Unir ambos dataframes
df_union = pd.merge(df_mujeres, df_total, on=['AÑO', 'PROGRAMA ACADÉMICO'], how='left').fillna(0)
...
y_pred = model.predict(X_test).flatten()   # Asegúrate de ejecutar esto justo después de entrenar
...
fig.update_layout(
    title="Comparación interactiva de tendencia real vs predicha (Red Neuronal)",
    xaxis_title="Casos (programas/años)",
